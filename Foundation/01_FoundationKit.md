...
**Figure 1-1. The Foundation Kit class inheritance hierarchy** *(diagram not included in source)*
...
`NSAutoreleasePools` are used to implement the delayed-release feature of the Foundation Kit, as described in "Object Ownership and Automatic Disposal."
...
**Figure 1-3. A More Complete Number Class Hierarchy** *(diagram not included in source)*
...
**Figure 1-4. Class Cluster Architecture Applied to Number Classes** *(diagram not included in source)*
...
{
    if (index >= [self count])
        [NSException raise:NSRangeException format:@"***%s: index (%d) beyond bounds (%d)",
...
**Figure 1-5. Embedding a Cluster Object** *(diagram not included in source)*
